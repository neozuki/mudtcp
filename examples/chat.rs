...
fn main() {
    ...
    let mut server = Server::<LineCodec>::new(addr.as_str())
        ...
        match ev {
            Event::Join(id) => {
                ...
                Event::Leave(id) => {
                    ...
                    let index = ids.iter().position(|_id|_id == id)
                        .expect("ID not found");
                    ids.remove(index);
                },
                Event::Receive(msg) => {
                    println!("Event: Received {} bytes from Client with id {}.",
                        msg.1.len(),
                        msg.0,
                        );
                    message_in.push(format!("Client[{}] says, \"{}\"",
                        msg.0,
                        msg.1));
                },
                Event::Send(_) => {
